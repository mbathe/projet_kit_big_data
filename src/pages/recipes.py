...
class Recipe:
    ...
    def initialize_session_state(self, name):
        if "data" not in st.session_state:
            st.session_state.data = load_dataset(
                dir_name=os.getenv("DIR_DATASET_2"), all_contents=True
            ).get(name)
    ...
    def preprocess_and_detect_anomalies(self):
        ...
        st.session_state.data["n_steps"] = st.session_state.data["n_steps"].astype(int)
        st.session_state.data["minutes"] = st.session_state.data["minutes"].astype(int)
        ...
        anomalies = pd.DataFrame(columns=["index", "column", "value", "reason"])
    ...
    def display_data_structures(self):
        ...
        if colonnes_preview:
            st.write(
                "Ce tableau fournit une description détaillée des colonnes utilisées dans la base de données des recettes. Chaque colonne contient des informations spécifiques permettant d’identifier et de décrire les recettes et leurs attributs."
            )
    ...
    def analyze_temporal_distribution(self):
        ...
        temporal_stats = {
            ...
            "submissions_per_year": df.groupby(df["submitted"].dt.year)
            .size()
            .to_dict(),
            "submissions_per_month": df.groupby(df["submitted"].dt.month)
            .size()
            .to_dict(),
            "submissions_per_weekday": df.groupby(df["submitted"].dt.dayofweek)
            .size()
            .to_dict(),
        }
        ...
        complexity_stats = {
            ...
            "time_stats": {
                ...
                "time_ranges": pd.cut(
                    ...
                    labels=["0-15min", "15-30min", "30-60min", "1-2h", ">2h"],
                )
                .value_counts()
                .to_dict(),
            },
...
with tabs[0]:
    option = st.selectbox(
        ...
        ["Description du dataset", "Colonne Ingredient", "Colonne Description"],
        key="selectbox_accueil",
    )
...
with tabs[1]:
    ...
    if option_analyse == "Distribution des soumissions":
        ...
        df_year = pd.DataFrame(
            list(submissions_per_year.items()), columns=["Année", "Soumissions"]
        )
        df_month = pd.DataFrame(
            list(submissions_per_month.items()), columns=["Mois", "Soumissions"]
        )
        df_weekday = pd.DataFrame(
            list(submissions_per_weekday.items()), columns=["Jour", "Soumissions"]
        )
        ...
        with tab1:
            ...
            st.dataframe(
                df_year.style.highlight_max(
                    subset=["Soumissions"], color="lightgreen"
        ...
        with tab2:
            ...
            st.dataframe(
                df_month[["Nom du Mois", "Soumissions"]]
        ...
        with tab3:
            ...
            st.dataframe(
                df_weekday[["Nom du Jour", "Soumissions"]]
                .style.highlight_max(subset=["Soumissions"], color="lightgreen")
                .highlight_min(subset=["Soumissions"], color="lightpink"),
                hide_index=True,
            )
        ...
        with tab1:
            ...
            df_steps = pd.DataFrame(
                list(steps_stats["distribution"].items()),
        ...
        with tab2:
            ...
            st.dataframe(
                df_time.style.highlight_max(
                    subset=["Nombre"], color="lightgreen"
        ...
        with tab1:
            ...
            subtab1, subtab2, subtab3 = st.tabs(["Par Année", "Par Mois", "Par Jour"])

            with subtab1:
                # Analyse par année
                df_year = pd.DataFrame(
                    list(submissions_data["submissions_per_year"].items()),
            ...
            with subtab2:
                # Analyse par mois
                df_month = pd.DataFrame(
                    list(submissions_data["submissions_per_month"].items()),
            ...
            with subtab3:
                ...
                df_weekday = pd.DataFrame(
                    list(submissions_data["submissions_per_weekday"].items()),
        ...
        with tab2:
            ...
            nutrient = st.selectbox(
                "Sélectionner un nutriment à analyser",
            ...
            if st.checkbox("Voir les statistiques détaillées"):
                st.dataframe(
                    df_nutrients.style.highlight_max(
                        subset=["Moyenne"], color="lightgreen"
        ...
        st.markdown("---")
        st.markdown("### 📊 Statistiques Globales")
        total_submissions = sum(submissions_data["submissions_per_year"].values())
        ...
        tags_data = {
            ...
            "tags_per_recipe": {"mean": 17.88, "median": 17.0, "min": 1, "max": 73},
        }
        ...
        with col4:
            st.metric(
                "Maximum de tags par recette", str(tags_data["tags_per_recipe"]["max"])
            )

        # Création des onglets
        tab1, tab2 = st.tabs(["🏷️ Tags les Plus Courants", "📊 Analyse Détaillée"])
        ...
        with tab1:
            ...
            fig_bars.update_traces(texttemplate="%{text:.1f}%", textposition="auto")
            ...
            if st.checkbox("Voir les données détaillées des tags"):
                st.dataframe(
                    df_tags.style.highlight_max(
                        subset=["Nombre d'utilisations"], color="lightgreen"
                    ).highlight_min(
                        subset=["Nombre d'utilisations"], color="lightpink"
                    ),
                    hide_index=True,
                )
        ...
        with tab2:
            ...
            fig_categories.update_traces(texttemplate="%{text:,}", textposition="auto")
        ...
        with col1:
            ...
            fig_dist.add_trace(
                go.Scatter(
                    x=x,
                    y=np.exp(-((x - tags_data["tags_per_recipe"]["mean"]) ** 2) / 50),
                    mode="lines",
        ...
        with col2:
            ...
            st.dataframe(
                stats_df.style.highlight_max(
                    subset=["Valeur"], color="lightgreen"
