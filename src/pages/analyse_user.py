...
class DataAnalyzer:
    ...
    def analyze_monthly_ratings(self):
        monthly_average_rating = (
            self.data.set_index("date").resample("ME")["rating"].mean().reset_index()
        )
...
class VisualizationManager:
    ...
    def display_line_chart(data, x, y, title):
        ...
        line_chart = LineChart(
            data=data, x=x, y=y, height=400, line_color="rgb(26, 28, 35)"
        )
        ...
        if self.data is not None:
            ...
            if "date" in self.data.columns and "rating" in self.data.columns:
                ...
                VisualizationManager.display_ratings_frequencies(
                    frequency_data,
                    x="year",
                    title="Fréquence des Notes au fil du temps",
                )
            ...
            if "user_id" in self.data.columns:
                ...
                if user_frequency_data:
                    VisualizationManager.display_ratings_frequencies(
                        user_frequency_data,
                        x="year",
                        title=f"Fréquence des Notes pour l'utilisateur {user_id}",
                    )
                else:
