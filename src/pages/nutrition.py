...
df_mean_rating = (
    df_RAW_interactions[["recipe_id", "rating"]].groupby(["recipe_id"]).mean()
)
# On ne garde que le nombre de notes de la recette
df_count_rating = (
    df_RAW_interactions[["recipe_id", "rating"]].groupby(["recipe_id"]).count()
)

df_nutrition = df_RAW_recipes[["id", "name", "nutrition"]]

merged_df = df_nutrition.merge(
    df_mean_rating, left_on="id", right_on="recipe_id"
).merge(df_count_rating, left_on="id", right_on="recipe_id")

merged_df.rename(
    columns={"rating_x": "Moyenne des notes", "rating_y": "Nombre de notes"},
    inplace=True,
)
...
valeurs_df = pd.DataFrame(merged_df["nutrition"].tolist(), index=merged_df.index)
...
calories_line = (
    final_df[["total_fat", "protein", "carbohydrates"]]
    .loc[final_df["name"] == selected_value]
    .iloc[0]
)

# Tracer le premier diagramme circulaire
axs[0].pie(calories_line, labels=calories_line.index, autopct="%1.1f%%", startangle=90)
...
axs[1].bar(
    full_line.index,
    full_line,
    color=["orange", "blue", "green", "red", "purple", "brown", "cyan"],
)
axs[1].axhline(
    100, color="red", linestyle="dashed", linewidth=2, label="Apport journalier requis"
)
...
correlation_matrix = final_df[
    ["Moyenne des notes", "total_fat", "protein", "carbohydrates", "sugar", "sodium"]
].corr()
