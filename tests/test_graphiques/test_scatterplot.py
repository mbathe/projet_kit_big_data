...
def test_scatterplot_initialization_and_display(mock_plotly_chart, sample_data):
    ...
    assert scatter_plot.data.equals(
        sample_data
    ), "Les données ne sont pas correctement initialisées."
    assert (
        scatter_plot.x == "Feature1"
    ), "L'attribut x n'est pas correctement initialisé."
    assert (
        scatter_plot.y == "Feature2"
    ), "L'attribut y n'est pas correctement initialisé."
    assert (
        scatter_plot.height == 400
    ...
    assert (
        mock_plotly_chart.called
    ), "La fonction plotly_chart de Streamlit n'a pas été appelée."
    assert (
        mock_plotly_chart.call_count == 1
    ), "La fonction plotly_chart a été appelée plus d'une fois."
    ...
    ), "La hauteur du graphique n'est pas correctement définie."
    assert (
        fig.layout.plot_bgcolor == "white"
    ), "La couleur de fond du graphique n'est pas 'white'."
    assert (
        fig.layout.paper_bgcolor == "white"
    ), "La couleur de fond du conteneur n'est pas 'white'."
...
def test_scatterplot_invalid_columns(sample_data, invalid_col):
    ...
    assert invalid_col in str(
        excinfo.value
    ), f"La colonne manquante {invalid_col} n'a pas été détectée."
