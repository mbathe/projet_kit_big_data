...
def test_histogramme_initialization(sample_data):
    ...
    assert histogram.data.equals(
        sample_data
    ), "Les données ne sont pas correctement initialisées."
    assert histogram.x == "Values", "L'attribut x n'est pas correctement initialisé."
    assert (
        histogram.height == 500
    ), "L'attribut height n'est pas correctement initialisé."
    assert (
        histogram.bin_size == 1
    ), "L'attribut bin_size n'est pas correctement initialisé."
...
def test_histogramme_afficher(mock_plotly_chart, sample_data):
    ...
    assert (
        mock_plotly_chart.call_count == 1
    ), "plotly_chart a été appelé plus d'une fois."
    ...
    ), "La hauteur de la figure n'est pas correctement définie."

    # Vérifie les propriétés du trace Histogram
    trace = fig.data[0]
    assert trace.type == "histogram", "Le type de trace n'est pas 'histogram'."
    assert (
        trace.x.tolist() == sample_data["Values"].tolist()
    ), "Les données de l'axe x ne correspondent pas."
    assert trace.xbins.size == 1, "La taille des bins n'est pas correcte."
    assert (
        trace.marker.color == "rgb(100, 149, 237)"
    ), "La couleur des barres n'est pas correcte."
    assert (
        trace.hovertemplate == "Intervalle: %{x}<br>Nombre: %{y}"
    ), "Le modèle de hover n'est pas correct."

    # Vérifie les mises à jour du layout
    layout = fig.layout
    assert (
        layout.plot_bgcolor == "white"
    ), "La couleur de fond du plot n'est pas 'white'."
    assert (
        layout.paper_bgcolor == "white"
    ), "La couleur de fond du papier n'est pas 'white'."
    ...
    ), "Les étiquettes de l'axe x ne sont pas affichées."
    ...
    ), "Les étiquettes de l'axe y ne sont pas affichées."
    ...
    ), "La couleur de la police des ticks de l'axe x n'est pas 'black'."
    assert (
        xaxis.titlefont.color == "black"
    ), "La couleur de la police du titre de l'axe x n'est pas 'black'."
    assert (
        yaxis.tickfont.color == "black"
    ), "La couleur de la police des ticks de l'axe y n'est pas 'black'."
    assert (
        yaxis.titlefont.color == "black"
    ), "La couleur de la police du titre de l'axe y n'est pas 'black'."
