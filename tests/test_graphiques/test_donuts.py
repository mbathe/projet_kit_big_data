...
def sample_data():
    ...
    data = pd.DataFrame({"Category": ["A", "B", "C", "D"], "Value": [30, 20, 25, 25]})
    return data
...
def test_donut_initialization(sample_data):
    ...
    assert donut.data.equals(
        sample_data
    ), "Les données ne sont pas correctement initialisées."
    assert (
        donut.names == "Category"
    ), "L'attribut names n'est pas correctement initialisé."
    assert (
        donut.values == "Value"
    ), "L'attribut values n'est pas correctement initialisé."
...
def test_donut_afficher(mock_plotly_chart, sample_data):
    ...
    assert (
        traces.hole == 0.4
    ), "Le trou de l'anneau (hole) n'est pas correctement défini."
    assert traces.hovertemplate == "%{percent}", "Le modèle de hover n'est pas correct."
    assert (
        traces.textinfo == "label"
    ), "Les informations de texte ne sont pas correctement définies."
