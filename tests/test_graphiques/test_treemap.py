...
def test_treemap_initialization_and_display(mock_plotly_chart, sample_data):
    ...
    treemap = Treemap(
        data=sample_data, path=["Category", "Subcategory"], values="Values", height=400
    )

    # Vérification des attributs
    assert treemap.data.equals(
        sample_data
    ), "Les données ne sont pas correctement initialisées."
    assert treemap.path == [
        "Category",
        "Subcategory",
    ], "L'attribut path n'est pas correctement initialisé."
    assert (
...
def test_treemap_invalid_columns(sample_data, invalid_col):
    ...
    for col in invalid_path + [invalid_values]:
        if col not in sample_data.columns:
            assert col in str(
                excinfo.value
            ), f"La colonne manquante {col} n'a pas été détectée."
